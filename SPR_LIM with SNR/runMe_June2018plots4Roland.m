...
int3 = plot(lambda, abs(broadRefP(3,:)).^2,'k');
int4 = plot(lambda, abs(broadRefP(4,:)).^2,'g');
...
phase3 = plot(lambda, angle(broadRefP(3,:)),'--k');
phase4 = plot(lambda, angle(broadRefP(4,:)),'--g');
ylabel('Phase (rad)')
ax.YTick = -pi:pi/2:pi;
set(ax,'yticklabels',{'-\pi','-\pi/2','0','\pi/2','\pi'})
...
title('Intensity (solid) and Phase (dashed) of P-polarised Reflection at 55º')
...
csvwrite('./plotsRoland_june2018/SPRrefData.csv',SPR_june2018)